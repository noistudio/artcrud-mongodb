        ...
        <div class="dzenkit-basic-card dzenkit-fulltable-maxwid-lg">
            ...
            <div class="dzenkit-basic-card-body rows_super_list">
                @foreach($[name_table_little] as $row)
                    <form style="display:none;" class="delete_{{ $row->last_id }}" action="{{ route('[ROUTE_NAME].destroy', $row->last_id) }}" method="POST">
                        @csrf
                        @method('DELETE')
                ...
                <form class="delete_form_all" action="{{ route('[ROUTE_NAME].delete_all') }}" method="GET">
                    <div class="table-responsive">
                        <table class="table">
                            ...
                            <tbody >
                            @foreach($[name_table_little] as $row)
                                <tr data-url-change="{{ route("[ROUTE_NAME].change",$row->last_id) }}" data-row-id="{{ $row->last_id }}" data-sort="{{ $row->sort }}">
                                    <td>
                                        <div class="row-nav d-flex">
                                            <div class="form-check">
                                                <input class="form-check-input form-check-number" type="checkbox"  name="id[]" value="{{ $row->last_id }}">
                                                <strong class="js_row_number">{{ $row->sort }}</strong></div>

                                            <div class="nav-bar d-flex align-items-center">
                                                <div class="change d-flex">
                                                    <button class="icon-down-big js_put_row_down" data-row-id="{{ $row->last_id}}"></button><button class="icon-up-big js_put_row_up" data-row-id="{{ $row->last_id}}"></button>
                                                </div>
                                                <div class="status">
                                                    <div class="form-check form-switch d-flex align-items-center">
                                                        <input class="form-check-input row_is_enable" data-route="{{ route("[ROUTE_NAME].toogle",$row->last_id) }}" type="checkbox" @if($row->enable==1) checked @endif >
                                                    </div>
                                                </div>
                                                <!-- action menu -->
                                                <div class="action">
                                                    <div class="dropleft">
                                                        ...
                                                        <ul class="dropdown-menu">
                                                            <li><a class="icon-pencil d-flex align-items-center" href="{{ route('[ROUTE_NAME].show', $row->last_id) }}" title="show">
                                                                    Просмотр
                                                                </a></li>
                                                            <li><a class="icon-pencil d-flex align-items-center" href="{{ route('[ROUTE_NAME].edit', $row->last_id) }}" title="show">
                                                                    Редактирование
                                                                </a></li>
                                                            <li><a class="icon-clone d-flex align-items-center" href="{{ route("[ROUTE_NAME].clone",$row->last_id) }}">Клонировать</a></li>
                                                            ...
                                                            <li><hr class="dropdown-divider"></li>
                                                            <li><a class="icon-trash-empty del d-flex align-items-center delete_row" data-row-id="{{ $row->last_id }}" href="#" data-toggle="modal" data-target="#deleteModal">Удалить</a></li>
                    ...
                    <nav>
                        {!! $[name_table_little]->links("artcrud_mg::paginate") !!}
...
@endsection
@section('footer_js')
    <script src="{{ asset("vendor/artcrud_mg/crud.js") }}"></script>
@endsection
